...
def noisify_mnist_symmetric(y_train, noise, random_state=None):
    P = np.eye(10)
    if noise > 0:
        P = build_uniform_P(10, noise)
...
def noisify_mnist_perm(y_train, noise, random_state=None):
    """mistakes:
        1 <- 7
        2 -> 7
        3 -> 8
...
def noisify_cifar10_asymmetric(y_train, noise, random_state=None):
    ...
    if n > 0.0:
        ...
        actual_noise = (y_train_noisy != y_train).mean()
        assert actual_noise > 0.0
        print('Actual total noise %.2f' % actual_noise)
        y_train = y_train_noisy

    return y_train, P


def noisify_cifar10_symmetric(y_train, noise, random_state=None):
    P = np.eye(10)
    if noise > 0.0:
        P = build_uniform_P(10, noise)
        y_train_noisy = multiclass_noisify(y_train, P=P,
...
def noisify_cifar100_symmetric(y_train, noise, random_state=None):
    """mistakes are inside the same superclass of 10 classes, e.g. 'fish'
    """
    P = np.eye(100)
    print(noise)
    if noise > 0.0:
        P = build_uniform_P(100, noise)
        print(P)
        y_train_noisy = multiclass_noisify(y_train, P=P,
                                           random_state=random_state)
        actual_noise = (y_train_noisy != y_train).mean()
...
if __name__ == '__main__':
    # print(build_uniform_P(3, 0.1))
    y_train, p = noisify_cifar100_symmetric(np.random.randint(0, 100, 1000), noise=0.6)
    print(y_train, )