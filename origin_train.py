...
from utils import mkdir_p,Logger,AverageMeter,Bar,accuracy
...
def main():
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        }, epoch,is_best,checkpoint=args.checkpoint,args=args)
...
def train(trainloader, model, train_criterion, optimizer, epoch, use_cuda,args):
    ...
    for batch_idx, (inputs, targets,idx) in enumerate(trainloader):
        ...
        bar.suffix = '({batch}/{size}) | ETA: {eta:} | Total: {total:.2f} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top1: .4f}'.format(
            batch=batch_idx + 1,
...
if __name__ == '__main__':
    main()
    '''test：lr
    optimizer, scheduler = config.get_optimizer(torch.nn.Sequential(torch.nn.Linear(3,3)))
    start_epoch = args.start_epoch
    lr_list=[]
    for epoch in range(start_epoch,args.epochs):
        scheduler.step()
